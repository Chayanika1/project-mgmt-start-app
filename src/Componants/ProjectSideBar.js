...
const ProjectSideBar = ({onStartAddProject,projects,onSelectProject,selectedProjectId}) => {
    return (
        <aside className='w-1/3 px-8 py-16 bg-stone-900 text-stone-50 md:w-72 rounded-r-xl'>
            ...
            {/* <button className='px-4 py-2 text-xs md:base rounded-md bg-stone-700 text-stone-400 hover:bg-stone-600 hover:text-stone-100'>+ AddProject</button> */}
            <Button onClick={onStartAddProject}>+ Add Project</Button>
            {/* <ul className='mt-8'>
                {
                    projects.map(project=><li key={project.id}>
                        <button className='w-full text-left px-2 py-1 rounded-sm my-1 text-stone-400 hover:text-stone-200 hover:bg-stone-800'onClick={onSelectProject}>{project.title}</button>
                    </li>)
                }
            </ul> */}
            {/* to show the selected project */}
            <ul className='mt-8'>
                {
